...
typedef struct {
    ...
    unsigned int dst_addr_ctrl : 2;  /*DstAddrCtrl :15:14  0:increment address 1: decrement address
                                        2: fixed address */
    unsigned int src_addr_ctrl : 2;  /*SrcAddrCtrl :17:16  0:increment address 1: decrement address
                                        2: fixed address */
 ...
 * @return    none
 * @note      When a certain dma channel has not finished the transmission (bit 0 of
 * reg_dma_ctr0(chn): 1-the transmission has not been completed,0-the transmission is
 * completed),need to disable dma before writing to the dma register.
 */
 ...
 * @param[in] chn - DMA channel
 * @param[in] size_byte  - the address of dma tx/rx size .The maximum transmission length of DMA is
 * 0xFFFFFC bytes  and cannot exceed this length.
 ...
 * completed),need to disable dma before writing to the dma register.
 */
static inline void dma_set_size(dma_chn_e chn, unsigned int size_byte,
                                dma_transfer_width_e byte_width)
{
    reg_dma_size(chn) =
        ((size_byte + byte_width - 1) / byte_width) | ((size_byte % byte_width) << 22);
}
 ...
 * @param[in]  chn - DMA channel
 * @param[in]  src_addr - the address of source.
 * @note When a certain dma channel has not finished the transmission (bit 0 of reg_dma_ctr0(chn) is
 * 1),need to disable dma before writing to the dma register
 */
