...
aduio_i2s_codec_config_t audio_i2s_codec_config = {
    ...
    .in_digital_gain = CODEC_IN_D_GAIN_16_DB,
...
#else
	.mic_input_mode_select = SINGLE_ENDED_INPUT,
#endif
    .dac_output_chn_select =
        DAC_OUTPUT_L_R_CHN, // 0 right and left channel both active ,1 only left channel active
};
 ...
 * @param[in]  en - I2S_DATA_INVERT_DIS: L channel ( left channel data left);
 * I2S_DATA_INVERT_EN(right channel data left)
 ...
 * @param[in]  en -lr_clk phase control(in RJ,LJ or i2s modes),in i2s mode(opposite phasing in RJ,LJ
 * mode), 0=right channel data when lr_clk high ,1=right channel data when lr_clk low. in DSP
 * mode(in DSP mode only), DSP mode A/B select,0=DSP mode A ,1=DSP mode B
 * @attention If the left and right channels are both active,there will be a phase difference(about
 * 1 sample) between the left and right channels,invert lr_clk can eliminate the phase
 * difference,but data output channel will invert.
 * @attention must be set before audio_init().
 * @return     none
 */
 ...
 *  @return     none
 */
void audio_set_codec_in_path_a_d_gain(codec_in_path_digital_gain_e d_gain,
                                      codec_in_path_analog_gain_e a_gain)
{
 ...
 *  @return     none
 */
void audio_set_codec_out_path_a_d_gain(codec_out_path_digital_gain_e d_gain,
                                       codec_out_path_analog_gain_e a_gain)
{
 ...
 * @param[in]  m_s - I2S_S_CODEC_M: i2s as slave ,codec as master; I2S_M_CODEC_S: i2s as  master,
 * codec  as slave.
 * @attention must be set before audio_init().
 * @return     none
 */
 ...
 * @param[in] chn-DAC_OUTPUT_L_R_CHN - right and left channel both active ; DAC_OUTPUT_L_CHN, only
 * left channel active.
 * @return    none
 * @attention must be set before audio_init().
 */
...
void aduio_set_chn_wl(audio_channel_wl_mode_e chn_wl)
{
    ...
    case MONO_BIT_16:
        ...
        audio_i2s_codec_config.dac_output_chn_select = DAC_OUTPUT_L_CHN;
        break;
 ...
 * @return    none
 */
void audio_rx_dma_config(dma_chn_e chn, unsigned short *dst_addr, unsigned int data_len,
                         dma_chain_config_t *head_of_list)
{
 ...
 * @return    none
 */
void audio_tx_dma_config(dma_chn_e chn, unsigned short *src_addr, unsigned int data_len,
                         dma_chain_config_t *head_of_list)
{
    /*DMA is not finished yet, need to disable dma before writing to the dma register.
     * confirmed by qiangkai,modify by minghai.duan(20211025)*/
 ...
 * @return    none
 */
void audio_tx_dma_add_list_element(dma_chain_config_t *config_addr, dma_chain_config_t *llpointer,
 ...
 * @return    none
 */
void audio_init(audio_flow_mode_e flow_mode, audio_sample_rate_e rate,
                audio_channel_wl_mode_e channel_wl)
{
    aduio_set_chn_wl(channel_wl);
    audio_set_codec_clk(1, 16); // from ppl 192/16=12M
    audio_mux_config(CODEC_I2S, audio_i2s_codec_config.audio_in_mode,
                     audio_i2s_codec_config.audio_in_mode, audio_i2s_codec_config.audio_out_mode);
    audio_i2s_config(I2S_I2S_MODE, audio_i2s_codec_config.i2s_data_select,
                     audio_i2s_codec_config.i2s_codec_m_s_mode, &audio_i2s_invert_config);
//    audio_set_i2s_clock(rate, AUDIO_RATE_EQUAL, 0);
    ...
    if (flow_mode > LINE_IN_TO_BUF) {
        audio_codec_dac_config(audio_i2s_codec_config.i2s_codec_m_s_mode, rate,
                               audio_i2s_codec_config.codec_data_select, MCU_WREG);
    }
    /* configure mux path */
...
void audio_i2c_init(codec_type_e codec_type, i2c_sda_pin_e sda_pin, i2c_scl_pin_e scl_pin)
{
    i2c_master_init();
    i2c_set_master_clk(
        (unsigned char)(sys_clk.pclk * 1000 * 1000 / (4 * 200000))); // set i2c frequency 200K.
 ...
 * @return    none
 */
void audio_init_i2c(audio_flow_mode_e flow_mode, audio_sample_rate_e rate,
                    audio_channel_wl_mode_e channel_wl)
{
    aduio_set_chn_wl(channel_wl);
    audio_set_codec_clk(1, 16); ////from ppl 192/16=12M
    ...
    audio_i2c_codec_write(addr_audio_codec_vic_ctr,
                          FLD_AUDIO_CODEC_SLEEP_ANALOG); // active analog sleep mode
 ...
 * @return    none
 */
void audio_codec_dac_config(i2s_codec_m_s_mode_e mode, audio_sample_rate_e rate,
                            codec_data_select_e data_select, codec_wreg_mode_e wreg_mode)
{
    ...
    if (wreg_mode == MCU_WREG) {
        ...
        BM_CLR(reg_audio_codec_vic_ctr,
               FLD_AUDIO_CODEC_SB |
                   FLD_AUDIO_CODEC_SB_ANALOG); // disable sb_analog,disable global standby
        ...
        if (audio_i2s_codec_config.dac_output_chn_select) {
            ...
            BM_SET(reg_audio_codec_dac_ctr,
                   FLD_AUDIO_CODEC_DAC_LEFT_ONLY); // active left channel only
        } else {
            BM_CLR(reg_audio_codec_dac_ctr,
                   FLD_AUDIO_CODEC_DAC_SB |
                       FLD_AUDIO_CODEC_DAC_LEFT_ONLY); // active DAC power,active left and right
                                                       // channel
        }
        ...
        reg_audio_codec_dac_freq_ctr =
            (FLD_AUDIO_CODEC_DAC_FREQ & (rate == AUDIO_ADC_16K_DAC_48K ? AUDIO_48K : rate));
        ...
        audio_i2c_codec_write(addr_audio_codec_dac_ctr,
                              MASK_VAL(FLD_AUDIO_CODEC_DAC_SB, 0, FLD_AUDIO_CODEC_DAC_LEFT_ONLY, 0,
                                       FLD_AUDIO_CODEC_DAC_SOFT_MUTE, 1));

        /*disable sleep mode ,disable sb_analog,disable global standby*/
        audio_i2c_codec_write(addr_audio_codec_vic_ctr,
                              MASK_VAL(FLD_AUDIO_CODEC_SB, 0, FLD_AUDIO_CODEC_SB_ANALOG, 0,
                                       FLD_AUDIO_CODEC_SLEEP_ANALOG, 0));

        /*data word length ,interface master/slave selection, audio interface protocol selection
         * ,active dac audio interface */
        audio_i2c_codec_write(
            addr_audio_codec_dac_itf_ctr,
            MASK_VAL(FLD_AUDIO_CODEC_FORMAT, CODEC_I2S_MODE, FLD_AUDIO_CODEC_DAC_ITF_SB,
                     CODEC_ITF_AC, FLD_AUDIO_CODEC_SLAVE, mode, FLD_AUDIO_CODEC_WL, data_select));

        /* disable DAC digital gain coupling, Left channel DAC digital gain */
        audio_i2c_codec_write(addr_audio_codec_dacl_gain,
                              MASK_VAL(FLD_AUDIO_CODEC_DAC_LRGOD, 0, FLD_AUDIO_CODEC_DAC_GODL,
                                       audio_i2s_codec_config.out_digital_gain));

        audio_i2c_codec_write(
            addr_audio_codec_dacr_gain,
            MASK_VAL(FLD_AUDIO_CODEC_DAC_GODR,
                     audio_i2s_codec_config.out_digital_gain)); /* Right channel DAC digital gain */

        /* disable Headphone gain coupling, set Left channel HP amplifier gain */
        audio_i2c_codec_write(addr_audio_codec_hpl_gain,
                              MASK_VAL(FLD_AUDIO_CODEC_HPL_LRGO, 0, FLD_AUDIO_CODEC_HPL_GOL,
                                       audio_i2s_codec_config.out_analog_gain));

        audio_i2c_codec_write(
            addr_audio_codec_hpr_gain,
            MASK_VAL(FLD_AUDIO_CODEC_HPR_GOR,
                     audio_i2s_codec_config
                         .out_analog_gain)); /*  Right channel HP amplifier gain programming*/
 ...
 * @return    none
 */
void audio_codec_adc_config(i2s_codec_m_s_mode_e mode, audio_input_mode_e in_mode,
                            audio_sample_rate_e rate, codec_data_select_e data_select,
                            codec_wreg_mode_e wreg_mode)
{
    ...
    if (wreg_mode == MCU_WREG) {
        ...
        if ((audio_i2s_codec_config.audio_in_mode == BIT_16_MONO) ||
            ...
            if (!audio_i2s_invert_config.i2s_data_invert_select) {
                BM_CLR(reg_audio_codec_adc12_ctr,
                       FLD_AUDIO_CODEC_ADC1_SB); /*active adc1 channel,mono.*/
            } else {
                BM_CLR(reg_audio_codec_adc12_ctr,
                       FLD_AUDIO_CODEC_ADC2_SB); /*active adc2 channel,mono.*/
            }
        } else {
            BM_CLR(reg_audio_codec_adc12_ctr,
                   FLD_AUDIO_CODEC_ADC1_SB |
                       FLD_AUDIO_CODEC_ADC2_SB); /*active adc1 and adc2  channel*/
        }
        BM_CLR(reg_audio_codec_vic_ctr,
               FLD_AUDIO_CODEC_SB | FLD_AUDIO_CODEC_SB_ANALOG |
                   FLD_AUDIO_CODEC_SLEEP_ANALOG); /*disable sleep mode ,disable sb_analog,disable
                                                     global standby*/

        if (in_mode == AMIC_INPUT) {
            /*Microphone 1 input selection ,Microphone biasing active,Single-ended input,MICBIAS1
             * output=2.08V,*/
            ...
            reg_audio_codec_mic2_ctr =
                MASK_VAL(FLD_AUDIO_CODEC_MIC2_SEL, 0, FLD_AUDIO_CODEC_MIC_DIFF2,
            ...
            reg_audio_dmic_12 = MASK_VAL(FLD_AUDIO_CODEC_ADC_DMIC_SEL2, 1,
                                         FLD_AUDIO_CODEC_ADC_DMIC_SEL1, 1, FLD_AUDIO_CODEC_DMIC2_SB,
        ...
        reg_audio_codec_adc_freq_ctr =
            MASK_VAL(FLD_AUDIO_CODEC_ADC12_HPF_EN, 1, FLD_AUDIO_CODEC_ADC_FREQ,
        ...
        audio_i2c_codec_write(addr_audio_codec_adc12_ctr,
                              MASK_VAL(FLD_AUDIO_CODEC_ADC1_SB, 0, FLD_AUDIO_CODEC_ADC2_SB, 0,
        ...
        if (in_mode == AMIC_INPUT) {
            ...
            audio_i2c_codec_write(addr_audio_codec_mic1_ctr,
                                  MASK_VAL(FLD_AUDIO_CODEC_MIC1_SEL, 0, FLD_AUDIO_CODEC_MICBIAS1_SB,
                                           0, FLD_AUDIO_CODEC_MIC_DIFF1, 0,
                                           FLD_AUDIO_CODEC_MICBIAS1_V, 0));

            /*Microphone 2 input selection,Single-ended input*/
            audio_i2c_codec_write(
                addr_audio_codec_mic2_ctr,
                MASK_VAL(FLD_AUDIO_CODEC_MIC2_SEL, 0, FLD_AUDIO_CODEC_MIC_DIFF2, 0));

            /*analog 0/4/8/12/16/20 dB boost gain*/
            audio_i2c_codec_write(
                addr_audio_codec_mic_l_R_gain,
            ...
            audio_i2c_codec_write(addr_audio_dmic_12,
                                  MASK_VAL(FLD_AUDIO_CODEC_ADC_DMIC_SEL2, 1,
        ...
        audio_i2c_codec_write(addr_audio_adc1_gain,
                              MASK_VAL(FLD_AUDIO_CODEC_ADC_LRGID, 1, FLD_AUDIO_CODEC_ADC_GID1,
                                       audio_i2s_codec_config.in_digital_gain));

        audio_i2c_codec_write(addr_audio_codec_adc_itf_ctr,
                              MASK_VAL(FLD_AUDIO_CODEC_FORMAT, CODEC_I2S_MODE,
                                       FLD_AUDIO_CODEC_SLAVE, mode, FLD_AUDIO_CODEC_WL,
                                       data_select));

        audio_i2c_codec_write(addr_audio_codec_adc2_ctr,
                              ~FLD_AUDIO_CODEC_ADC12_SB); /*audio adc interface active*/

        /*  adc high pass filter active, set adc sample rate   */
        audio_i2c_codec_write(
            addr_audio_codec_adc_freq_ctr,
 ...
 * @return    none
 */
void audio_mux_config(audio_flow_e audio_flow, audio_in_mode_e ain0_mode, audio_in_mode_e ain1_mode,
                      audio_out_mode_e i2s_aout_mode)
{
    reg_audio_ctrl |= audio_flow;
    reg_audio_tune = MASK_VAL(FLD_AUDIO_I2S_I2S_AIN0_COME, ain0_mode, FLD_AUDIO_I2S_I2S_AIN1_COME,
                              ain1_mode, FLD_AUDIO_I2S_I2S_AOUT_COME, i2s_aout_mode);
}
 ...
 * @param[in] i2s_config_t - the prt of i2s_config_t that configure i2s lr_clk phase and lr_clk
 * swap. i2s_config_t->i2s_lr_clk_invert_select-lr_clk phase control(in RJ,LJ or i2s modes),in i2s
 * mode(opposite phasing in  RJ,LJ mode), 0=right channel data when lr_clk high ,1=right channel
 * data when lr_clk low. in DSP mode(in DSP mode only), DSP mode A/B select,0=DSP mode A ,1=DSP mode
 * B. i2s_config_t->i2s_data_invert_select - 0=left channel data left,1=right channel data left.
 * @attention:If the left and right channels are both active in i2s mode,there will be a phase
 * difference(about 1 sample) between the left and right channels,you can set
 * i2s_lr_clk_invert_select=1 to eliminate the phase difference, but data output channel will be
 * inverted,you can also set i2s_config_t->i2s_data_invert_select=1 to recovery it.
 * @return    none
 */
void audio_i2s_config(i2s_mode_select_e i2s_format, i2s_data_select_e wl, i2s_codec_m_s_mode_e m_s,
                      audio_i2s_invert_config_t *i2s_config_t)
{

    reg_i2s_cfg = MASK_VAL(FLD_AUDIO_I2S_FORMAT, i2s_format, FLD_AUDIO_I2S_WL, wl,
 ...
 * @return    none
 * @attention i2s clock  divider from pll,sampling rate calculation is based on pll=192M,so pll must
 * be 192M
 */
_attribute_ram_code_sec_noinline_ void audio_set_i2s_clock(audio_sample_rate_e audio_rate,
                                                           audio_rate_match_e match,
...
void audio_rx_dma_chain_init(dma_chn_e chn, unsigned short *in_buff, unsigned int buff_size)
{
    ...
    /* the next LLP is own address; therefore, DMA uses the same configure all the time */
    audio_rx_dma_add_list_element(&g_audio_rx_dma_list_cfg, &g_audio_rx_dma_list_cfg,
                                  (unsigned short *)in_buff, buff_size);
    audio_rx_dma_en();
}
...
void audio_tx_dma_chain_init(dma_chn_e chn, unsigned short *out_buff, unsigned int buff_size)
{
    ...
    audio_tx_dma_add_list_element(&g_audio_tx_dma_list_cfg, &g_audio_tx_dma_list_cfg,
                                  (unsigned short *)out_buff, buff_size);
    audio_tx_dma_en();
}
...
void pwm_set(pwm_pin_e pin)
{
    pwm_set_pin(pin);
    pwm_set_clk(
        (unsigned char)(sys_clk.pclk * 1000 * 1000 / 24000000 - 1)); // set pwm clk equal pclk 24M
...
void audio_codec_dac_power_down(void)
{
    ...
    reg_audio_codec_vic_ctr = MASK_VAL(FLD_AUDIO_CODEC_SB, CODEC_ITF_PD, FLD_AUDIO_CODEC_SB_ANALOG,
                                       CODEC_ITF_PD, FLD_AUDIO_CODEC_SLEEP_ANALOG, CODEC_ITF_PD);
    audio_tx_dma_dis();
    audio_clk_en(0, 0);
}
...
void audio_codec_dac_power_on(void)
{
    ...
    reg_audio_codec_vic_ctr = MASK_VAL(FLD_AUDIO_CODEC_SB, CODEC_ITF_AC, FLD_AUDIO_CODEC_SB_ANALOG,
                                       CODEC_ITF_AC, FLD_AUDIO_CODEC_SLEEP_ANALOG, CODEC_ITF_AC);
    audio_set_codec_dac_unmute(); // dac unmute
    audio_tx_dma_en();
}
