...
/*The Wind Noise filter (WNF) is a programmable high pass filter feature enabling to reduce wind
noise . The wind noise filter is a 1st order filter. Mode1  -3dB   59Hz corner frequency   Mode2
-3dB   117Hz Mode3  -3dB   235Hz
*/
typedef enum
{
...
{
    /* i don't understand why you configure double size of len */
    reg_rx_max = ((len) >> 2) - 1;
}
...
/**
 * @param[in] buff is the start address
 */
static inline unsigned int audio_get_tx_rptr(dma_chn_e chn, void *buff, int type_len)
{
    return ((audio_get_tx_dma_rptr(chn) - (unsigned int)buff) / type_len);
}

static inline unsigned int audio_get_rx_wptr(dma_chn_e chn, void *buff, int type_len)
{
    return ((audio_get_rx_dma_wptr(chn) - (unsigned int)buff) / type_len);
}
 ...
 *  @return     none
 */
static inline void audio_write_codec_ind_reg(unsigned char reg, unsigned char addr,
                                             unsigned char data)
{
...
/**
 * @brief  This function serves to set wind noise filter(WNF),it is a programmable high pass filter
 * feature enabling to reduce wind noise.
...
static inline void audio_set_codec_adc_a_gain(codec_in_path_analog_gain_e a_gain)
{
    ...
    reg_audio_codec_mic_l_R_gain =
        MASK_VAL(FLD_AUDIO_CODEC_AMIC_L_GAIN, a_gain, FLD_AUDIO_CODEC_AMIC_R_GAIN, a_gain);
}
...
static inline void audio_set_codec_dac_a_gain(codec_out_path_analog_gain_e a_gain)
{
    ...
    reg_audio_codec_hpl_gain =
        MASK_VAL(FLD_AUDIO_CODEC_HPL_LRGO, 0, FLD_AUDIO_CODEC_HPL_GOL, a_gain);
    reg_audio_codec_hpr_gain =
        MASK_VAL(FLD_AUDIO_CODEC_HPR_GOR, a_gain); /* Right channel HP amplifier gain programming*/
}
...
static inline void audio_set_codec_dac_d_gain(codec_out_path_digital_gain_e d_gain)
{
    ...
    reg_audio_codec_dacl_gain =
        MASK_VAL(FLD_AUDIO_CODEC_DAC_LRGOD, 0, FLD_AUDIO_CODEC_DAC_GODL, d_gain);
    reg_audio_codec_dacr_gain =
        MASK_VAL(FLD_AUDIO_CODEC_DAC_GODR, d_gain); /*Right channel DAC digital gain*/
}
