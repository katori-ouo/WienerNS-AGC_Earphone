    ...
    (MIC_SAMPLE_RATE == 8000)                                                            \
        ...
        : ((MIC_SAMPLE_RATE == 16000)                                                    \
               ...
               : ((MIC_SAMPLE_RATE == 32000)                                             \
                      ? AUDIO_32K                                                        \
                      : ((MIC_SAMPLE_RATE == 48000) ? AUDIO_48K : AUDIO_16K)))
...
/**
 * @brief		This function serves to handle  iso in usb endpoint interrupt ,interval is 1
 * ms
 * @param[in]   none
 * @return      none
 */
...
void user_init(void)
{
    ...
    // set in path digital gain,analog gain does not work, must be set before
    // audio_init(),;
...
#define DMIC_2_DEC_FOR_DEC0                                                              \
    2 // The data of dec0 and dec1 are stored in one buff,only process dec0 data( the
      // second 4bytes in the buff).
#define DMIC_2_DEC_FOR_DEC1                                                              \
    3 // The data of dec0 and dec1 are stored in one buff,only process dec1 data( the
      // first 4bytes in the buff).
...
#if (!((DMIC_INPUT_MODE == DMIC_2_DEC_FOR_DEC0) ||                                       \
       ((DMIC_INPUT_MODE == DMIC_2_DEC_FOR_DEC1))))
    for (unsigned char i = 0; i < length; i++) {
        short md = iso_in_buff[iso_in_r++ & (MIC_BUFFER_SIZE - 1)];
        reg_usb_ep7_dat = md;
        reg_usb_ep7_dat = md >> 8;
...
void user_init(void)
{
    ...
    audio_rx_dma_chain_init(DMA0, (unsigned short *)iso_in_buff, MIC_BUFFER_SIZE * 2,
                            FIFO0);
#elif ((DMIC_INPUT_MODE == DMIC_2_DEC_FOR_DEC0) ||                                       \
       (DMIC_INPUT_MODE == DMIC_2_DEC_FOR_DEC1))
    audio_set_codec_clk(1, 16);
    reg_codec_adc_1m_clk_ctr |= FLD_AUDIO_CODEC_DMIC_CLK_MODE;
    audio_set_codec_dec0_sample_rate(MIC_SAMPLING_RATE);
    ...
    audio_data_fifo0_path_sel(CODEC_DEC1_IN_FIFO, OUT_NO_USE);
    audio_dec0_in_mux_config(CODEC_BIT_16_STEREO, CODEC_BIT_16_STEREO);
    audio_dec1_in_mux_config(CODEC_BIT_16_STEREO_DEC0_DEC1,
                             CODEC_BIT_16_STEREO_DEC0_DEC1);
    audio_set_dec0_vol(CODEC_IN_D_GAIN_42_DB); // dec0  codec vol
    audio_set_dec1_vol(CODEC_IN_D_GAIN_42_DB); // dec1 codec vol
    audio_set_dmic0_pin(GPIO_PA0, GPIO_PA1);
