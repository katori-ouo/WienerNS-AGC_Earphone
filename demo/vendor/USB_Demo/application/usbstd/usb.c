...
void usb_handle_out_class_intf_req(int data_request)
{
    ...
    case CDC_REQ_SetControlLine_State:
        ...
        break;
    case CDC_REQ_SetLine_Encoding: //--2-2. set serial parameter:baudrate,check,stop and
                                   // data
        ...
        if (USB_IRQ_DATA_REQ == data_request) {
            ...
            //-todo- according to LineCoding from USB HOST, user can set baud rate, stop
            // bits, parity, data bit, etc.
        }
        break;
...
void usb_handle_request(unsigned char data_request)
{
    ...
    case (REQDIR_HOSTTODEVICE | REQTYPE_STANDARD | REQREC_INTERFACE): // 01
        if (REQ_SetInterface == Request) { // 0x0b
            usb_handle_set_intf();
        } else if (REQ_SetFeature == Request) { // 0x03
            g_stall = 1;
            usb_g_feature = 1;
        }
        break;
...
void usb_handle_ctl_ep_data(void)
{
    ...
    if (g_stall) {
        usbhw_write_ctrl_ep_ctrl(FLD_EP_DAT_STALL);
    } else if ((usb_len_idx_s % 8 == 0) && (usb_len_idx_0 == 0) &&
               (usb_len_idx_s != usb_len_idx_h)) {
...
#endif
#if (USB_CDC_ENABLE)
        // must add ,if endpoint is reset and ack is not set,CDC out_irq will not be
        // generated. The packet capture phenomenon of the USB analyzer is: device does
        // not return ack.(kaixin modify,2020-01-15)
...
//    usbhw_enable_manual_interrupt(FLD_CTRL_EP_AUTO_STD | FLD_CTRL_EP_AUTO_DESC | FLD_CTRL_EP_AUTO_CFG);
    ...
    usbhw_set_eps_en(FLD_USB_EDP8_EN | FLD_USB_EDP1_EN | FLD_USB_EDP2_EN |
                     FLD_USB_EDP3_EN | FLD_USB_EDP4_EN | FLD_USB_EDP5_EN |
