...
unsigned char ble_tx_packet[48] __attribute__((aligned(4))) = {
    3, 0, 0, 0, 0, 10, 0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 0x7, 0x8, 0x9, 0xa, 0xb, 0xc, 0xd, 0xc, 0xf};
...
_attribute_ram_code_sec_ void stimer_irq_handler()
{
    if (stimer_get_irq_status(FLD_SYSTEM_IRQ)) {
        ...
        stimer_set_irq_capture(stimer_get_tick() + 2000 * SYSTEM_TIMER_TICK_1MS);
        delay_us(250 * 1000);
...
_attribute_ram_code_sec_ void timer0_irq_handler(void)
{
    ...
    if (timer_get_irq_status(TMR_STA_TMR0)) {
        ...
        //stimer_set_irq_capture(stimer_get_tick() + 2000 * SYSTEM_TIMER_TICK_1MS);
        timer_set_cap_tick(TIMER0, sys_clk.pclk * 1000 * 1000 * 2); // 1ms
        delay_us(250 * 1000);
...
_attribute_ram_code_sec_ void except_handler()
{
    ...
    case TRAP_M_STACKOVF:
        /*sp may be greater than bound, because mcu will make a judgment,
         ...
         it will goto overflow exception, and the corresponding sp has not been updated. confirm by
         minghai*/
